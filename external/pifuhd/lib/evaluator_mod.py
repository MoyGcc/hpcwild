...
class MeshEvaluator:
    ...
    def set_mesh(self, src_path, tgt_path, scale_factor=1.0, offset=0):
        self.src_mesh = trimesh.load(src_path, process=False)
        self.tgt_mesh = trimesh.load(tgt_path, process=False)
        # self.src_mesh.vertices *= (self.tgt_mesh.scale / self.src_mesh.scale) 
        # self.src_mesh.vertices = self.src_mesh.vertices - self.src_mesh.vertices.mean(0)
        # self.tgt_mesh.vertices = self.tgt_mesh.vertices - self.tgt_mesh.vertices.mean(0)
    ...
    def get_chamfer_dist(self, num_samples=10000):
        ...
        # self.tgt_mesh.vertices *= self.scale_factor
        # self.tgt_mesh.vertices[:,2] += 0.12
        # self.tgt_mesh.vertices[:,1] -= 0.32
        # self.tgt_mesh.vertices[:,0] += 0.08
...
if __name__ == '__main__':
    ...
    def run(root, exp_name, tar_path):
        ...
        for index in range(1):# trange(len(buff_files)) :
            file = buff_files[index]
            tar_name = os.path.join(bf_path, file)
            actor_id, name, frame = tar_name.split('/')[6], tar_name.split('/')[-1][:-4], int(tar_name.split('/')[-1][-10:-4])
        
            for src in src_names:
                src_name = os.path.join(src_path, 'result_%s_%s_%d_%s' % (actor_id, name[:-7], frame, src))

                if not os.path.exists(src_name):
                    continue
                # evaluator.set_mesh(src_name, tar_name, 0.13, -40)
                evaluator.set_mesh(src_name, tar_name, 1/1000, -40)
                vals = []
                vals.append(1e2 * evaluator.get_chamfer_dist(num_samples=10000))
                vals.append(1e2 * evaluator.get_surface_dist(num_samples=10000))
        ...
        vals = np.array(total_vals).mean(0)
        print('BUFF - chamfer: %.4f  p2s: %.4f ' % (buf_val[0], buf_val[1]))
