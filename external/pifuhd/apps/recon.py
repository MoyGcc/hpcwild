...
def recon(opt, name, frame, use_rect=False):
    # load checkpoints
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start_id, end_id)):
            ...
            if True:
                ...
                save_path = '%s/%s/recon/result_%s_%s_%d.obj' % (opt.results_path, opt.name, name, str(frame), opt.resolution)
                # save_path = '%s/%s/recon/BUFF/%s/%s/result_%s_%s_%d.obj' % (opt.results_path, opt.name, name[:5], name[6:], name, str(frame), opt.resolution)
                # if not os.path.exists('%s/%s/recon/BUFF/%s/%s' % (opt.results_path, opt.name, name[:5], name[6:])):
                #     os.makedirs('%s/%s/recon/BUFF/%s/%s' % (opt.results_path, opt.name, name[:5], name[6:]))
                
                # save_path = '%s/%s/recon/3DPW/%s/result_%s_%s_%d.obj' % (opt.results_path, opt.name, name[5:], name, str(frame), opt.resolution)
                # if not os.path.exists('%s/%s/recon/3DPW/%s' % (opt.results_path, opt.name, name[5:])):
                #     os.makedirs('%s/%s/recon/3DPW/%s' % (opt.results_path, opt.name, name[5:]))

                # save_path = '%s/%s/recon/iPER/%s/result_%s_%s_%d.obj' % (opt.results_path, opt.name, name[5:], name, str(frame), opt.resolution)
                # if not os.path.exists('%s/%s/recon/iPER/%s' % (opt.results_path, opt.name, name[5:])):
                #     os.makedirs('%s/%s/recon/iPER/%s' % (opt.results_path, opt.name, name[5:]))
                print(save_path)
